...
public class TypedCallbackHandlerMap {
    ...
    public <T extends CallbackHandler> void put(Class<? extends Callback> clazz, T callbackHandler) {
        map.put(clazz, callbackHandler);
    }

    public <T> T get(Class<? extends Callback> clazz, Class<T> handlerType) {
        return handlerType.cast(map.get(clazz));
    }

    public void putAll(TypedCallbackHandlerMap typedCallbackHandlerMap) {
        if (typedCallbackHandlerMap != null) {
            map.putAll(typedCallbackHandlerMap.map);
        }
    }
