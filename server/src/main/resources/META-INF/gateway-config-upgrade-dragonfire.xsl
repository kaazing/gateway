...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template
		match="/dragonfire:gateway-config/dragonfire:management[position() &lt; last()]">
		<xsl:comment>
			Unused management configurations have been removed.
		</xsl:comment>
	</xsl:template>

	<!-- Remove all but the last network entry -->
	<xsl:template
		match="/dragonfire:gateway-config/dragonfire:network[position() &lt; last()]">
		<xsl:comment>
			Unused network configurations have been removed.
		</xsl:comment>
	</xsl:template>
	...
	<xsl:template match="dragonfire:*">
		<xsl:element name="g:{local-name()}"
			namespace="http://xmlns.kaazing.com/gateway-config/excalibur">
			<xsl:apply-templates />
		</xsl:element>
	</xsl:template>
