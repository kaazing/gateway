...
public class MultipleMaskPropertiesTest {

    @Rule
    public TestRule testExecutionTrace = new MethodExecutionTrace();
    
    @Test
    public void shouldFailGatewayStartupWhenMultipleMaskPropertiesSet() throws Exception {
        ...
        try {
            ...
            throw new AssertionError("Gateway should fail to start because there are multiple masking keys defined.");
        } catch (NumberFormatException e) {
            Assert.assertTrue("Wrong error message: " + e.getMessage(), e.getMessage().matches(
                    "For input string: \"1010101,0x1010101\""));
        } finally {
            gateway.stop();
        }
    }
