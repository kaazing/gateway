 ...
 * occur until either select() is called or when it is blocked and woken up. So we modified
 * this class so that on close the selector is woken up. In order to make sure we don't mark
 * the close future as completed until it is actually close we use a future that gets marked
 * completed in the process method.
 */
