...
/**
 * Utilities useful at a couple levels of the management support. In particular, both
 * the SessionManagementBean and the ServiceManagementBean want to send information
 * about the type, family and direction (accept/connect/...) of a given session over
...
public class Utils {
    ...
    /** 
     * Create a descriptive string for the type of connection that this
     * session has been created on. This is generally just used in the Command Center.
     * The following is the same as the insides of ResourceAddress.getProtocol(),
     * which is private.
     * NOTE: we're using BridgeSession.LOCAL_ADDRESS.get(session) to retrieve the
     * address to send to management.
     */
    ...
    /**
     * Determine the session 'direction' (accept/connect/???). We need this to
     * be determined just once, so that it hangs around after the session is closed.
     */
    ...
    /**
     * Given futures from the various ThreadServiceStats, wait for them to finish and 
     * add their numeric values together. The Futures are all Future<Object> so we
     * don't have to have lots of different ones, so we coerce types here.
     * @param futures
     * @return
     */
    ...
    /**
     * Given an array of strings, return a string that equals the output of creating
     * a JSONArray and then stringifying it.
     * @param values
     * @return
     */
    ...
    /**
     * Convert a server-side string into either an OctetString (if the string is non-null)
     * or Null.instance ("the value of this OID is null") for transmission.
     * This is so the client side can tell the difference between real nulls and empty values.
     * 
     * @param s
     * @return
     */
    ...
    /**
     * Convert a server-side byte array into either an OctetString (if the array is non-null)
     * or Null.instance ("the value of this OID is null") for transmission.
     * This is so the client side can tell the difference between real nulls and empty values.
     * 
     * @param s
