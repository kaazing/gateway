...
/**
 * Filter for session filter chains. Supports 'strategies' to tailor the amount of processing
 * based on whether management is desired or not at the time. Strategies that actually create
 * a SessionManagementBean store the bean as a session attribute (see IoSession and AttributeKey).
 * All the strategies are stateless and final--they get the sessionManagementBean that they're 
 * operating on from the incoming IoSessionEx.
 * 
 * The global strategy is changed from the DefaultManagementContext. As it can be changed at
 * any time, rather than keep it in each ManagementFilter and require it to be updated from DMC,
 * each ManagementFilter will just request it each time it needs to do something.
 * 
 * There is exactly 1 ManagementFilter per service instance (excluding the management services,
 * for which we do not want any management).
 */
public class ManagementFilter extends IoFilterAdapter<IoSessionEx> {
    ...
    protected void doSessionClosed(NextFilter nextFilter, IoSessionEx session) throws Exception {
        managementContext.getManagementFilterStrategy().doSessionClosed(managementContext, serviceBean, session.getId(), Utils.getManagementSessionType(session));
    ...
    protected void doMessageReceived(NextFilter nextFilter, IoSessionEx session, Object message) throws Exception {
        managementContext.getManagementFilterStrategy().doMessageReceived(managementContext, serviceBean, session.getId(), session.getReadBytes(), message);
    ...
    protected void doFilterWrite(NextFilter nextFilter, IoSessionEx session, WriteRequest writeRequest) throws Exception {
        managementContext.getManagementFilterStrategy().doFilterWrite(managementContext, serviceBean, session.getId(), session.getWrittenBytes(), writeRequest);
    ...
    protected void doExceptionCaught(NextFilter nextFilter, IoSessionEx session, Throwable cause) throws Exception {
        managementContext.getManagementFilterStrategy().doExceptionCaught(managementContext, serviceBean, session.getId(), cause);
    ...
    public void newManagementSession(IoSessionEx session) throws Exception {
        ...
        managementContext.getManagementFilterStrategy().doSessionCreated(managementContext, serviceBean, session, Utils.getManagementSessionType(session));
    }
}
