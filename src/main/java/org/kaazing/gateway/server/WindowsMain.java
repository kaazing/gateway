 ...
 * Java code as well.) This particular version is for Windows, where the 
 * directories are set up to be directly under GATEWAY_HOME, and we preprocess
 * the argument list to 'canonicalize' it so it looks like the standard
 * command line, but also so we specialize help messages.
 * 
 * As of 3.2 we support two arguments to main:
 *    /config=<configFile>
 *    /help
 *    (we also support the form /config:configFile and -config=configFile
