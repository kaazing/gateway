...
public class GatewayConfigParser {
    ...
    public GatewayConfigDocument parse(final File configFile) throws Exception {
        ...
        if (xmlParseErrors.isEmpty()) {
            // The properties used in parameter substitution are now proper XMLBeans and should be injected after an initial parse
            ...
            if ( LOGGER.isTraceEnabled() ) {
                xmlInjectedIn = bufferToTraceLog(xmlInjectedIn, "Gateway config file '" + configFileName + "' post parameter injection", LOGGER);
            }
            ...
            if ( LOGGER.isTraceEnabled() )
                xmlTransformedIn = bufferToTraceLog(xmlTransformedIn, "Gateway config file '" + configFileName + "' post XSL transformation", LOGGER);
                ...
                if (services != null && services.length > 0) {
                	...
                	for (ServiceType service : services) {
                		...
                		if (name == null || name.length() == 0) {
                			...
                			errorList.add(XmlError.forMessage("Service name must be unique. More than one service named '" + name + "'",
                					XmlError.SEVERITY_ERROR));                			
                		} else {
        ...
        if (errorList.size() > 0) {
            ...
            for (XmlError error : errorList) {
                ...
                if (error.getMessage().contains("DataRateString")) {
                    ...
                    // ERROR -   string value '1m' does not match pattern for DataRateString in namespace http://xmlns.kaazing.com/2012/08/gateway
    ...
    private static final class XMLParameterInjector implements Callable<Boolean> {
        ...
        public XMLParameterInjector(InputStream souceInput, OutputStream injectedOutput, Map<String, String> properties, Properties configuration, List<String> errors)
            ...
            try {
                ...
                DefaultHandler handler = new DefaultHandler2() {
                    ...
                    public void startDocument() throws SAXException {
                         write("<?xml version=\"1.0\" encoding=\"" + CHARSET_OUTPUT_XML + "\" ?>" + System.getProperty("line.separator"));
                    }
