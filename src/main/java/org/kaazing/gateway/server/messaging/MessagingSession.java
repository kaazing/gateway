 ...
 * 
 * TODO: Transactions are not yet supported.
 * TODO: May want to fire a messageSent event on the session for send acknowledgement.
 */
public interface MessagingSession extends AttributeStore {
     ...
     * 
     * selector - no selector, any filtering is done by protocol handlers or clients of messagingprovider
     * ack_mode - no ack mode, all messages must be acknoledged by the client
     * no_local - resolved by session.getId() and msg.getSender() compare
     * is_durable - placed on the release for now. this will require us to pay for replication for all subscriptions. This could be changed.
     * 
     * @param destination
     * @param attributes
     * @param listener
     */
    void acquireSubscription(String destination, Map<String, Object> attributes, MessagingHints hints, MessagingSubscriptionListener listener);
