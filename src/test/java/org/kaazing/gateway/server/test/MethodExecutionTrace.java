...
/**
 * This class can be used to print out a message at the start and end of each test method in a JUnit test class by including the
 * following at the start of the class:
 *
 * @Rule public MethodRule testExecutionTrace = new MethodExecutionTrace();
 * <p/>
...
public class MethodExecutionTrace extends TestWatchman {
    ...
    /**
     * Use this constructor from integration tests that do not actually start up the gateway but which need to set up a particular
     * log4j configuration using a properties file, e.g. to use MemoryAppender. See gateway.server
     * src/test/resources/log4j-trace.properties for an example.
     *
    ...
    public void failed(Throwable e, FrameworkMethod method) {
        if (e instanceof AssumptionViolatedException) {
            System.out.println(String
                    .format("%s skipped programmatically with reason: %s", getFullMethodName(method), e.getMessage()));
        } else {
