...
public class WsrBindingsTest {
    ...
    public void tearDown() throws Exception {
        ...
        sslAcceptor.dispose(); tcpAcceptor.dispose();
    ...
    public void shouldBindAndUnbindLeavingEmptyBindingsMaps() throws Exception {
        ...
        for ( int iterationCount: rounds ) {
            for ( int i = 0; i < iterationCount; i++) {
                wsrAcceptor.bind(bindAddress, ioHandler, null);
            }
            for (int j = 0; j < iterationCount; j++) {
                UnbindFuture future = wsrAcceptor.unbind(bindAddress);
                org.junit.Assert.assertTrue("Unbind failed", future.await(10, TimeUnit.SECONDS));
            }
            org.junit.Assert.assertTrue(wsrAcceptor.emptyBindings());
